---
id: entity-soren
record_type: entity
schema_version: 1.0.0
lifecycle_state: active
created_at: 2026-07-10T00:00:00+02:00
updated_at: 2026-07-10T15:39:48+02:00
provenance: initialized during campaign scaffold generation; character sheet linked by explicit human instruction during Session 2 processing review on 2026-07-10; updated by approved Session 3 canonization on 2026-07-10; updated by approved Session 4 canonization on 2026-07-10; updated by approved Session 5 canonization on 2026-07-10; updated by explicit human instruction on 2026-07-10 to preserve persistent carried item details
campaign_ref: campaign-echoes-of-tal-dorei
entity_kind: player-character
primary_name: Soren
entity_status: active
aliases: []
current_state: Cleric of Shar back on the true road to Emon with the party, Sadek, and Perry; current sheet linked; retains the Lantern of Lost Souls, which allows him to speak with the dead once per day, by explicit human instruction; one defining childhood memory remains but its emotional charge has been removed; consumed the Hourglass of Returning to save Sadek, received Shar's final miracle to stop Kallista's erasure, and is now an independent Time Agency operative when called upon.
character_sheet_ref: ASSETS/pdf/player-character-sheets/soren.pdf
relationship_refs:
  - relationship-soren-researches-with-phirina
timeline_refs:
  - event-session-001-caravan-departure
  - event-session-001-fog-discontinuity
  - event-session-001-false-emon-arrival
  - event-session-002-settlement-instability
  - event-session-002-jarek-gifts
  - event-session-002-mage-tower-manifestation
  - event-session-002-temporal-agent-warning
  - event-session-003-soren-pact-with-shar
  - event-session-003-rune-hideout-discovery
  - event-session-003-stone-guardian-awakens
  - event-session-004-stone-keeper-defeated
  - event-session-004-underground-city-revealed
  - event-session-004-hourglass-used-on-sadek
  - event-session-005-undercity-revelations
  - event-session-005-vael-confronted
  - event-session-005-arven-revealed-and-rune-breaks
  - event-session-005-relic-stand-against-paradox-wyrm
  - event-session-005-timeline-stabilized-and-agency-recruitment
asset_refs: []
notes:
---

# Soren

## Current State

Cleric who has shifted from Sarenrae to Shar, saved Sadek, preserved Kallista from erasure, and now carries both the cost and the memory of the collapsed prologue branch into the real timeline.

## Relationships

- Research partnership with Phirina.

## Timeline

- Joined the caravan to Emon at Emerald Outpost.
- Witnessed the fog discontinuity and the false arrival at Emon.
- Investigated the sudden market and the map-seller clues with Phirina.
- Entered the Mage Tower and agreed to investigate the forest ruins.
- Accepted Shar's bargain during the trial of compassion.
- Lost the emotions tied to one defining childhood memory while retaining the memory itself.
- Helped defeat the Stone Keeper of Judgment.
- Used the Hourglass of Returning to save Sadek's life.
- Reached the entrance of the forgotten underground city.
- Crossed the undercity and learned Sadek's survival had broken Vael's fixed-point calculations.
- Received Shar's final miracle to prevent Kallista from being erased during the Paradox Wyrm battle.
- Returned to the restored timeline and accepted future work with the Time Agency.

## Notes

Current character sheet PDF linked. Canon now preserves his research partnership with Phirina, his permanent shift to Shar, his decision to expend the Hourglass to save Sadek, his Session 5 miracle against Kallista's erasure, his Time Agency recruitment, and the explicit human instruction that he still retains the Lantern of Lost Souls and can use it to speak with the dead once per day.
