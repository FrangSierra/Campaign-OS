---
id: entity-phirina
record_type: entity
schema_version: 1.0.0
lifecycle_state: active
created_at: 2026-07-10T00:00:00+02:00
updated_at: 2026-07-10T21:10:00+02:00
provenance: initialized during campaign scaffold generation; character sheet linked by explicit human instruction during Session 2 processing review on 2026-07-10; updated by approved Session 3 canonization on 2026-07-10; updated by approved Session 4 canonization on 2026-07-10; updated by approved Session 5 canonization on 2026-07-10
campaign_ref: campaign-echoes-of-tal-dorei
entity_kind: player-character
primary_name: Phirina
entity_status: active
aliases: []
current_state: Scholar-mage back on the true road to Emon with the party, Sadek, and Perry; current sheet linked; remembers the collapsed prologue branch, sacrificed the Last Page of the Archivist for True Revelation during the Paradox Wyrm battle, and is now an independent Time Agency operative when called upon.
character_sheet_ref: ASSETS/pdf/player-character-sheets/phirina.pdf
relationship_refs:
  - relationship-soren-researches-with-phirina
timeline_refs:
  - event-session-001-caravan-departure
  - event-session-001-fog-discontinuity
  - event-session-001-false-emon-arrival
  - event-session-002-settlement-instability
  - event-session-002-jarek-gifts
  - event-session-002-laughing-lamia-revelations
  - event-session-002-mage-tower-manifestation
  - event-session-002-temporal-agent-warning
  - event-session-003-keeper-trials
  - event-session-003-rune-hideout-discovery
  - event-session-003-stone-guardian-awakens
  - event-session-004-stone-keeper-defeated
  - event-session-004-underground-city-revealed
  - event-session-005-undercity-revelations
  - event-session-005-vael-confronted
  - event-session-005-arven-revealed-and-rune-breaks
  - event-session-005-relic-stand-against-paradox-wyrm
  - event-session-005-timeline-stabilized-and-agency-recruitment
asset_refs: []
notes:
---

# Phirina

## Current State

Scholar and mage whose curiosity drove the party through the prologue branch and who now carries direct knowledge of how false realities can be built, broken, and perceived.

## Relationships

- Research partnership with Soren.

## Timeline

- Joined the caravan to Emon at Emerald Outpost.
- Witnessed the fog discontinuity and the false arrival at Emon.
- Recognized the constellation-symbol links between Elira's clues and later discoveries.
- Solved the four-symbol puzzle that manifested the Mage Tower.
- Agreed to investigate the forest ruins after hearing the Temporal Agent's warning.
- Completed the trial of knowledge only by admitting she did not know the answer.
- Helped uncover the hidden evidence about the Rune's pre-collapse study.
- Helped defeat the Stone Keeper of Judgment.
- Argued against spending the Hourglass of Returning before the party descended further.
- Reached the entrance of the forgotten underground city.
- Witnessed mirrored timelines in the undercity and learned the ruins were built for containment.
- Sacrificed the Last Page of the Archivist to perceive the truth behind Vael, Arven, and the collapsed timelines.
- Returned to the restored timeline and accepted future work with the Time Agency.

## Notes

Current character sheet PDF linked. Canon now preserves her symbol-puzzle work, her Session 3 knowledge trial, her Session 5 True Revelation, her Time Agency recruitment, and her approved research relationship with Soren.
